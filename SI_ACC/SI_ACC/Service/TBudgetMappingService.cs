...
public class TBudgetMappingService
{
    ...
    public async Task<BudgetMappingResponse> GetAllAsync(string? filter)
    {
        try
        {
            ...
            {
                await BatchEndOfDay_BudgetMappingBySearch(filter);
    ...
    public async Task BatchEndOfDay_BudgetMappingBySearch(string? filter)
    {
        var options = new JsonSerializerOptions
        {
            PropertyNameCaseInsensitive = true,
    ...
    public Task<TBudgetMapping?> GetByIdAsync(string budgetid, string mappingCode, int mappingparentcode) => _repository.GetByIdAsync(budgetid, mappingCode, mappingparentcode);
